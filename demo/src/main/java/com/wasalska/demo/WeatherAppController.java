...
public class WeatherAppController implements Initializable {
    ...
    private final ObservableList<Weather> data =
            FXCollections.observableArrayList(
                    new Weather("Krakow", 20.0,4.0, 3),
                    new Weather("Londyn", 10.0, 0.0,4),
                    new Weather("Graz", 23.0, 5.8,0)
            );

    @FXML
    public void handleRefresh(ActionEvent actionEvent) {
        titleLabel.setText("Udalo sie");
        data.add(new Weather("Praha", 38, 4.5, 4));
        ...
        Weather[] weather = weatherJsonToWeather(result);
    ...
    Weather[] weatherJsonToWeather(WeatherJson[] weatherJson){
        ...
        for (int i = 0; i < weatherJson.length; i++){
            weather[i] = new Weather(weatherJson[i].getStacja(), weatherJson[i].getTemperatura(), weatherJson[i].getPrecipitation(), weatherJson[i].getWind());
        }
        return weather;
    }
